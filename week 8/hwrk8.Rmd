...
-   $\frac{1}{n}\sum_{i}Z_i \sim \mathcal{N}(E[Z],\sqrt{\frac{V[Z]}{n}}) $
...
```{r}
coverage <- function(data, estimator,n=100, rep = 10){
  true_ratio <- estimator(data(100000))
  samples <- map_df(1:rep,function(.x){
    dataset <- slice_sample(data(n),n = nrow(data(n)),replace = T)
    return(bootstrap_estimator(data= dataset,estimator, B=100))
  }
  )
  samples %>% 
...
test <- coverage(DGP_ratio,estimator)
test
```
