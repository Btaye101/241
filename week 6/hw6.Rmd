...
-   3rd bullet: Since $P(U=1\mid D=1,X) = 0$ and $P(U=0 \mid D=1,X) = 1$ we can substitute these values in resulting in: $E[Y\mid D=1,X] = E[Y\mid D=1,X,U=1]*0+E[Y\mid D=1,X,U=0]*1$ which simplifies to $E[Y\mid D=1,X]=E[Y\mid D=1,X,U=0]$. The wizard knows from the DGP that $Y\mid X,U$ is normally distributed with mean $X^2 + U$ so with U being 0 and it being normally distributed its mean parameter is the expectation, $E[Y\mid D=1,X,U=0] = X^2 + 0 = X^2$. So $E[Y\mid D=1,X] = \mu(X)$.
